...
const Patchnote = ({
  ...
  isPatchnoteOpen: boolean
  onTogglePatchnote: () => void
}) => {
  const [patchnotes] = useState<Patchnote[]>([
    { id: 1, createdAt: '2025-12-25', version: '1.0.0', content: '안녕하세요. 김쥠님입니다. 잘 부탁드립니다.' },
    { id: 2, createdAt: '2025-12-26', version: '1.0.1', content: '검색 기능을 추가했습니다.' },
  ])

  return (
    <>
      <div className={`patchnote-container scrollbar-custom ${isPatchnoteOpen ? 'open' : 'closed'}`}>

        <button
          className="patchnote-toggle-btn"
          ...
          <ul className="pb-list">
            {patchnotes.slice().reverse().map((note) => (
              <li key={note.id} className="pb-item">
                <strong className="pb-item-title">김쥠님 {note.version} 업데이트 안내</strong>
      ...
      {isPatchnoteOpen && <div className="patchnote-overlay" onClick={() => onTogglePatchnote()}></div>}
    </>
  )
}
